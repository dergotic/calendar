...
class Calendar extends React.Component {
    ...
    for (let i = 0; i < byDate.length; i++) {
      ...
      days.push(
        <button key={i} className={className + ' tooltip'}>
          {i + 1}
          <time dateTime={'2025-08-' + i + 1} className={byDate[i].start_date ? 'tooltiptext' : ''}>
  ...
  render() {
    ...
    return (
      <main>
        <label>
          <input type="checkbox" defaultChecked={showPendingRegistrations} onChange={this.handleChangeChk} />
